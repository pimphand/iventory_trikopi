...
<div class="flex justify-between items-center mb-6">
    <h4 class="text-slate-900 dark:text-slate-200 text-lg font-medium">Produk</h4>
</div>
<!-- Page Title End -->
<div class="card">
        ...
        <div class="overflow-x-auto">
            <div class="min-w-full inline-block align-middle">
                <button class="btn bg-primary text-white mb-2" type="button" id="open" data-fc-target="default-modal"
                    onclick="$('#title').text('Tambah Produk');$('#form-input')[0].reset()" data-fc-type="modal"
                    type="button">
                    Tambah Produk
                </button>
                    ...
                    <div class="py-3 px-4 flex">
                        <div class="relative max-w-xs">
                            ...
                            <input type="text" name="table-with-pagination-search" id="table-with-pagination-search"
                                class="form-input ps-11" placeholder="Cari produk">
                        ...
                        <div class="relative max-w-xs px-2">
                            <select type="text" name="table-with-pagination-search" id="table-with-pagination-search"
                                class="form-select" placeholder="Search for items">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700" id="tabel-head">
                                <tr>
                                    ...
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kategori
                                    </th>
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Stok
                                        Terakhir
                                    </th>
                                    <th scope="col"
    ...
    <div
        ...
        <form id="form-input">
            <div class="px-4 py-8 overflow-y-auto">
                ...
                <div class="grid grid-cols-1 gap-6 py-2">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Pilih
                            Kategori</label>
                        <select type="text" name="category_id" id="category_id" class="form-select">
                            <option value="">Semua Kategori</option>
                            @foreach ($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                ...
                <div class="grid grid-cols-1 gap-6 py-2">
                    <div>
                        <label for="quantity" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Stock
                            Awal</label>
...
<script>
    ...
    let data = null;
    function get() {
        requestData('get', '{{ route('products.create') }}', null, function(error, response) {
        if (error) {
            console.log(error);
        } else {
                $('#data-table').empty();
                $.each(response.data, function (index, item) {
                    $('#data-table').append(`
                        <tr>
                            ...
                            <td class="px-6 py-3 whitespace-nowrap">
                                <div class="text-sm text-gray-900">${item.stock.quantity}</div>
                            </td>
                            <td class="px-6 py-3 whitespace-nowrap">
    ...
    $('#form-input').submit(function(e) {
        ...
        requestData('post', '{{ route('products.store') }}', formData, function(error, response) {
            if (error) {
                console.log(error);
            } else {
                get();
    ...
    $(document).on('click', '.deleted', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                requestData('delete', '{{ route('products.index') }}/' + id, null, function(error, response) {
                    if (error) {
                        console.log(error);
                    } else {
                        get();
